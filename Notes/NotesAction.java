...
public class NotesAction { // Notes.Notes.NotesAction class
    ...
    // Method to add notes to the Notes.Notes object
    ...
    // Method to withdraw notes from the Notes.Notes object
    // This method is static because it does not depend on instance-specific data and can be shared across all instances
    public static boolean withdrawNotes(Notes notes, double amount) {
        ...
        if (amount == 0) {
            System.out.println("Withdrawal successful. Notes.Notes dispensed: ");
            displayWithdrawnNotes(withdrawnNotes); // Display the withdrawn notes
            return true; // Return true if withdrawal is successful
        } else {
            notes.getNotes().addAll(withdrawnNotes); // Restore the withdrawn notes back to the Notes.Notes object if exact amount cannot be withdrawn
            return false; // Return false if withdrawal is unsuccessful
        }
    }

    // Helper method to remove a note of a specific value from the Notes.Notes object
    ...
    // Helper method to check if a note of a specific value exists in the Notes.Notes object
    ...
    // Method to convert Notes.Notes object to a string representation
    // This method is static because it does not depend on instance-specific data and can be shared across all instances
